...
const FilterPill = memo(
  ({ label, emoji, isActive, color, bg, onPress }: PillProps) => {
    ...
    return (
      ...
      >
        {!!emoji && <AppText style={styles.pillEmoji}>{emoji}</AppText>}
        <AppText
          variant="caption1"
...
const FoodCatalogScreen = memo(() => {
  ...
  const { data, isLoading, refetch, isRefetching } = useFoodCatalog(
    showFavsOnly ? undefined : params,
  );
  ...
  const {
    ...
    isPending: isTogglingAny,
  } = useToggleFavourite();

  const displayedFoods: FoodItem[] = showFavsOnly
    ? favourites ?? []
    : data?.foods ?? [];
  ...
  return (
    <Screen padded={false} safeArea={false}>
      ...
      >
        ...
        {/* ── Search bar ── */}
        <AppView
          style={[styles.searchRow, { backgroundColor: colors.background }]}
        >
          <AppView
            style={[
              styles.searchBox,
            ...
            return (
              <FilterPill
                ...
                onPress={() => {
                  ...
                  setDietFilter(d === dietFilter ? 'all' : d);
                }}
              />
            );
          })}
